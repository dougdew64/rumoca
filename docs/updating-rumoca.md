...
compiler and the test suite do most of the work; two artifacts need explicit regeneration.
...
## 5. Refresh `docs/understanding/` — only if phases changed, and only by Doug
...
- `cargo run`; load a specimen; confirm each stage renders, and the bridge capture / field-help
  panel / "Go to" navigation / debugger arming still work.

## 7. Update the pin references
...
**Rule of thumb:** steps 1–3 are mechanical and compiler-driven; step 4 is one command; steps 5
and 7 are human judgement. If `cargo build` + `cargo test` are green and the field-help diff looks
