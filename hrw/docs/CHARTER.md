...
**It is generative rather than cosmetic, which is the test it had to pass.** A lab has a notebook, pre-lab questions, apparatus, a protocol, observations and a writeup. HRW already has `specimen-notebook/`, predictions, panes and routes. **The vocabulary therefore predicts what is missing instead of merely renaming what exists**, and the kinds become self-explanatory: an *adjudication tour* explains nothing, while **calibration against a reference standard** explains itself; failure tours become **fault diagnosis**, and feature tours become **equipment orientation**.
