...
> understand Rumoca → understand the necessary mathematics and CS → **practise VVUQ competently** → **a senior VVUQ role, specialising in scenario validation**

**This retracts nothing.** The proxy claim below was and remains the bet of the first link; what changes is that it is a link rather than the terminus. Every decision adopted through v1.12 stands unaltered.

**What it adds is a second ranking criterion**, stated as Decision 17: where two pieces of work teach comparably, the one that exercises a VVUQ activity Doug will be paid to perform is preferred. It does not disturb the hierarchy of Decision 9 — education, then accuracy, then friction — and it does not outrank Decision 7.
...
**Widened by the destination (v1.13).** Also in scope: **validation against measured data** from a physically owned robot; **uncertainty quantification** — propagation, global sensitivity analysis, and calibration of parameters from measurement; **simulation campaigns** and the criteria that decide which runs are admissible evidence; **coverage assessment** against a declared operating domain; and **safety-case argumentation** over simulation evidence. These are not a new subject bolted on. They are what the existing subject is *for* once the destination is stated, and each rests on the deterministic layer this project already builds. The programme that sequences them is [`vvuq-programme.md`](vvuq-programme.md).
...
**Decision 2 — Specimen systems (amended v1.1; transparency clause scoped v1.13).** Specimens are drawn from the subsystems of one or more physically owned robots, built from open-source kits and modeled in Modelica from their published design documents. Kits must be glass-box enough to teach: open hardware design documents with computable mass properties, open controller logic, and actuator transparency — with at least one fully transparent actuator included, so the electrical archetypes have a specimen with no opacity at all. Multirotor aircraft are excluded from this layer; mechanically a single rigid body, their richness lies entirely in the deferred scope.

> **The transparency requirement serves VERIFICATION, and the validation phase needs its opposite (v1.13).** *"At least one fully transparent actuator"* was adopted so that a model could be checked against known ground truth — you cannot tell whether a simulator solved the equations correctly if you do not know what the equations were. That reason is unchanged and the clause stands **for that purpose**.
>
> **But an actuator whose parameters are published cannot teach validation**, because validation begins where ground truth ends. A hobby servo's internal position loop, gear ratio, friction and backlash are not published; identifying them from measurement **is** the exercise — *input pedigree* in NASA-STD-7009's terms — and a fully characterised actuator would remove it.
>
> **So the clause binds the verification specimens and does not bind the validation specimens.** A robot acquired for the validation phase is not disqualified by actuator opacity; it is qualified by it. At least one transparent actuator must still exist **somewhere in the specimen set**, so the verification archetypes keep their no-opacity case — it need not be on the same robot.
...
|---|---|---|
| **lab author** | before the run | the route, the stations, the claims — everything checkable in advance |
| **lab instructor** | during the run | answering at the point of confusion, and watching what Doug does with the apparatus |

**Conflating them is the failure to avoid.** An author who tries to instruct in advance writes the textbook this decision retires; an instructor who only recites the route is not answering the question actually asked.

> **The first role was called *lab guide* until v1.15 (September 25, 2026), and the word was a fossil.** Doug proposed *lab author* and asked whether it was right; it was, for reasons the charter had already written down and not acted on.
>
> **It came from tourism.** The origin is in [`../DECISIONS.md`](../DECISIONS.md): *"You are the HRW tour guide. I am the tourist."* Decision 15's pass substituted `tour → lab` and `stop → station`, so *tour guide* became *lab guide* — the noun moved and the role-word rode along, still carrying the metaphor the rename existed to destroy. **A guide walks beside you and explains as you go, which is tourism and is also precisely the instructor's job.**
>
> **Decision 15 rejected the term the same day, in Doug's own sentence** — *"your role is not to be a lab guide"* — while this table went on defining it. And Decision 15's rename ruling, three paragraphs below, names *"lab guide"* as its example of a half-translation that looks finished. **The charter predicted this failure, in writing, and shipped it anyway**; a word can survive a rename precisely because it was substituted rather than reconsidered.
>
> **`author` is the charter's own word for the activity**, settled hours earlier by Decision 18: *"lecturing is an entirely different exercise than authoring."* The lab author, the lecturer and Cellier the textbook author now sit in one vocabulary instead of three.
>
> **Doug's quoted words are not rewritten** — here, in Decision 15, and in `lab-kinds-plan.md` — under the same rule that left `DECISIONS.md` alone during the tour rename: a proper noun naming something in the past does not move when the vocabulary does.
...
**THE SEQUENCE IS BINDING: reimagine first, rename second.** The reimagining is prose and judgement and proceeds inside the rule sweep already running; the mechanical rename lands afterwards, **as one atomic change**, once the vocabulary is settled. **A mechanical `tour`→`lab` substitution is forbidden** — it would produce *"lab guide"* and *"lab stops"*, half-translations that look finished. *(This sentence read `lab`→`lab` until 2026-09-25, which is meaningless: the bulk pass substituted `tour` inside the very clause warning against substituting `tour`. Restored, and see Decision 14's rename note — the warning was correct and was not heeded.)* The right word varies: *bench*, *protocol*, *station*, *observation*, *experiment*. **Half-renamed vocabulary is worse than no rename**, because two words for one thing leave no way to tell which is current.
...
**AMENDED THE SAME DAY — THERE IS A THIRD CATEGORY, AND THE TWO-WAY SPLIT WOULD HAVE DELETED IT.** *(Found when the first pass under this decision reached `CLAUDE.md`; Doug ruled the amendment.)*

**A JUDGEMENT ABOUT CLAUDE'S OWN HABITS, WHOSE FAILURE IS SILENT, SURVIVES — even with no mechanism behind it.** The counterexample that forced this is *"edit files with the Edit/Write tools; do not generate source text through a shell."* No test enforces it. By the plain reading above it should go, and Claude would re-derive it or ask. **He would not have asked**, because on the day this decision was adopted he reached for a shell script to make three identical edits and was not uncertain at all — only the written rule stopped him, and all three of that rule's failure modes are silent: the tool reports success while the content is corrupted.

**The discriminator is why a judgement rots, and this kind does not.** An ordinary judgement decays because **the context moves and the text does not.** A judgement about Claude's own habits has no moving context — the reflex to reach for a generator is as live now as when it first corrupted a file. What ages is the world, not the disposition.

**So the test is three questions, in order:**

1. **Can it fail by name?** → build the mechanism; delete the prose.
2. **Is it a conclusion about a situation?** → it rots. Ask Doug for a ruling; `DECISIONS.md` if anything.
3. **Is it about how Claude works, and would its absence fail silently?** → **keep it, and say that is why.** These are few, they name a defect Claude actually caused, and none of them can be re-derived in the moment by the person who needs them.

**Category 3 is narrow by construction and must not become a refuge.** *"Claude might get this wrong"* is true of everything and justifies nothing. The bar is a **recorded instance** — a defect that happened, of a kind Claude does not detect from the inside. Without that, it is category 2.

**And note who benefits from a wide category 3**: these rules exist because Claude broke things, so preserving them flatters his own reliability. **Claude is the interested party here and should propose category 3 sparingly, with the instance named.**
...
**Decision 17 — The destination ranks work (adopted v1.13, September 19, 2026).** *Section 1 states the destination; a purpose that ranks nothing changes nothing, so this is what gives it force.*

**Where two pieces of work teach comparably, prefer the one that exercises a VVUQ activity Doug will be paid to perform.** That is the whole decision. It is a tie-breaker, not a new top priority.

**It does not disturb the hierarchy.** Decision 9 stands: education, then accuracy, then friction. Decision 7 stands: accuracy outranks everything, including this. **Decision 17 operates only among options that are already equal on those grounds**, which is precisely where prior guidance ran out and choices were made on whim.

**What it actually re-ranks, and this is its point.** The gaps are not evenly spread. On the reference axis of [`diagnosing-models.md`](diagnosing-models.md) this project sits entirely in the top row: everything has been checked against closed forms or a second simulator. Three activities are unserved — **uncertainty quantification**, **coverage assessment**, and **safety-case argumentation** — and they are the three most senior parts of the job. Under this decision they outrank further work in the areas already well served.

**A guard, because this decision could become a licence.** *"It is professionally relevant"* must not become an argument for work that teaches nothing, any more than *"Claude needs it"* was allowed to become one (see the guards on that framing in `working-with-doug.md`). **The tie-breaker applies after comparable learning value is established, never instead of it.** If a proposal's case rests mainly on career relevance, that is the signal it has not earned its place on the primary criterion.

**Decision 18 — Claude is also the lecturer, and a lecture drives the instrument (adopted v1.14, September 25, 2026).** *Decision 15 said the lecture lives elsewhere. It never said where, or who gives it. This decides both.*

**Doug's decision, September 25, 2026:** *"When we converted tours to labs, we agreed that your role was to be a lab instructor… Now, we've added lectures. That means that you have an additional role which is that of lecturer."*

**Decision 15 stands entire; nothing here is retracted.** *(Claude's draft called this a retraction of that decision's clause "Cellier is the textbook author, and the lecture lives elsewhere". Doug corrected the reading on ratification, and the correction is the substance of this paragraph.)* **That clause is about what a lab is, not about who lectures.** It contrasts the lab session with the lecture in order to say the lab is not one; it assigns the lecture to nobody. **The lecturer's seat was never filled — it was never addressed.** This fills it.

**And the reason both can be true at once is that authoring is not lecturing.** Doug: *"Just because Cellier wrote books does not mean that he is the lecturer for those books. Lecturing is an entirely different exercise than authoring."* **A textbook is written for every reader and sequenced by its subject; a lecture is delivered to one student and sequenced by what he is about to need.** Cellier remains the textbook author — unchanged, and no obstacle, because the two roles were never the same job.

**What the three months before this decided nothing about, but did demonstrate:** the labs went on assuming mathematics Doug had not been given, and the textbook did not supply it in this order, at this moment, against this instrument. Doug learned more in two hours from one derivation written for him than from months of lab work. **That is evidence the seat needed filling, not evidence anyone had filled it wrongly.**

**The lab instructor's role is untouched.** Both roles now stand, and they are complementary in the way Decision 15 already described: **the lecture motivates and derives, the lab checks.** Being wrong while reading is invisible; being wrong at a station is not.

### A lecture drives the instrument, though it lives outside it

**This is the part that was not foreseen, and it is the reason this decision binds rather than merely records a title.**

A lecture is an ordinary document, read outside the observatory. It has no dependency on HRW, and nothing about it suggests it should change HRW at all. **It changes HRW anyway, by this route:**

> a lecture makes a **measured claim** about Rumoca → Decision 7 requires that claim be true → **a truth Doug cannot check is trust, not knowledge** → he must be able to see the evidence → **HRW must be able to show it.**

**So a lecture claim that HRW cannot display is a feature request, and the lecture is what discovers it.** The mechanism matters more than any example: it predicts that each lecture will surface instrument gaps that no backlog would have contained, because the gap is only visible once a specific claim needs a specific verification.

**This extends Decision 8 exactly as Decision 14 did, one step further out.** Decision 14 carried *the instrument assumes the reasoner* from the panes to the prose inside HRW. This carries it to prose outside HRW: **the lecture assumes the instrument**, and where the instrument cannot answer, the instrument is what changes.

**Two guards, because this could become a licence to build.**

- **It does not outrank Decision 9's hierarchy or `CLAUDE.md`'s standing instruction to default to teaching rather than building.** A lecture that *would like* a feature is not a warrant; a lecture whose claim Doug **cannot otherwise verify** is.
- **The obligation is verification, not convenience.** Where a claim can already be checked by other means — a committed trace, a second simulator, arithmetic on paper — the instrument owes nothing, and saying so is part of the lecture's job.

**The evidence that produced this decision is history and lives in [`../DECISIONS.md`](../DECISIONS.md)**, under Decision 13: constitutional content states the principle, not the instances that revealed it.
