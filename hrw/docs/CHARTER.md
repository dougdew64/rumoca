...
**Decision 14 — A walk is a lab session, not a reading (adopted v1.10, September 1, 2026).** *Extends Decision 8 from the panes to the prose, and retires the model the tour rules were written for.*
...
**This is not a new idea, which is the argument for it.** [`vision.md`](vision.md) already holds that **the RHS is a lab, not an illustration**, and Decision 8 already holds that Doug assembles the *noun* by pointing while the *verb* is his own utterance. The panes have been a lab for a month; the tours were still a textbook. The 🎯 capture is Decision 8 realised for prose, and this decision is the consequence nobody had drawn.

**AND THE HAZARD IT CREATES, which is binding.** A conversational walk delivers Claude's errors at the moment Doug is most receptive to them, in the register most likely to be believed — the *effective but false* quadrant, which he cannot detect precisely because it landed. **So Decision 7 governs here without exception: the tour's claims stay machine-checked even though Claude is present to answer.** Claude's presence is not a substitute for a checker; it is the reason one is needed.

**Decision 15 — Tours are labs, and Claude is the lab instructor (adopted v1.11, September 1, 2026).** *Decision 14 established what the activity is; this one gives it its name, and the name is not cosmetic.*

**Doug's decision, September 1, 2026:** *"if what we have been calling tours are actually labs, then your role is not to be a tour guide. Instead, your role is to be a lab instructor. And that role complements the textbook author and lecturer roles which academics like Cellier play."* **The concept of a tour is replaced, not relabelled.**

**Why the word was working against us.** *Tour* encodes passivity: a guide talks continuously, the visitor looks but does not touch, and the guide is never surprised. That is exactly the model Decision 14 retired, so the vocabulary had begun to contradict the decision. *Lab* encodes **apparatus, protocol, prediction, observation, and the possibility of being wrong** — all of which already exist in HRW under names that fit them worse.
...
**It is generative rather than cosmetic, which is the test it had to pass.** A lab has a notebook, pre-lab questions, apparatus, a protocol, observations and a writeup. HRW already has `specimen-notebook/`, predictions, panes and routes. **The vocabulary therefore predicts what is missing instead of merely renaming what exists**, and it makes at least one existing name self-explanatory: an *adjudication tour* explains nothing, while **calibration against a reference standard** explains itself.

**CORRECTION, September 1, 2026 — this decision originally offered two further examples that do not survive contact with the codebase**, and they are recorded here so no later session re-attempts them. It proposed *fault diagnosis* for failure tours and *equipment orientation* for feature tours. **`diagnosis` collides with Rumoca's compiler diagnostics** (`diagnostic` appears 804 times in `crates/`), and **`orientation` collides with rotational mechanics** (184 in `crates/`), which robotics makes worse rather than better. A third candidate for concept tours, *experiment*, is **worse still: `experiment(...)` is a Modelica annotation**, present in fourteen specimen files and required by `CLAUDE.md`'s comparison protocol — a collision with the language being taught.

**So only `calibration` was adopted, and `concept`, `feature` and `failure` keep their names.** *The general point outlives the three words: this decision requires renaming what carries the **tour metaphor** — `tour`, `stop`, `guide` — and those three names never did. Renaming beyond that is aesthetic, and is not worth buying with a domain collision. Collision-check every candidate against `src/`, `crates/` and `specimens/` before proposing it; the examples above were written without doing so.*
...
**THE SEQUENCE IS BINDING: reimagine first, rename second.** The reimagining is prose and judgement and proceeds inside the rule sweep already running; the mechanical rename lands afterwards, **as one atomic change**, once the vocabulary is settled. **A mechanical `tour`→`lab` substitution is forbidden** — it would produce *"lab guide"* and *"lab stops"*, half-translations that look finished. The right word varies: *bench*, *protocol*, *station*, *observation*, *experiment*. **Half-renamed vocabulary is worse than no rename**, because two words for one thing leave no way to tell which is current.

**Two rulings, both Doug's, September 1, 2026:**

- **Every `hrw://tour/` link is rewritten atomically. There is no permanent alias.** *"We are replacing the concept of tours with the concept of labs."* `fixture_tour_links_all_resolve` makes this safe: a missed link fails by name rather than rotting.
