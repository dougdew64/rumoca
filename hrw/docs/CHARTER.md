...
**Decision 18 — Claude is also the lecturer, and a lecture drives the instrument (drafted by Claude September 25, 2026; AWAITING DOUG'S RATIFICATION).** *Decision 15 gave Claude the lab instructor's role and, in the same breath, gave the lecture away. This takes it back, and records what that turned out to cost the instrument.*
...
**This retracts a clause of Decision 15, which is why it is an amendment rather than an addition.** That decision states outright that it *"names what Claude is not — Cellier is the textbook author, and the lecture lives elsewhere."* **The lecture does not live elsewhere.** It was tried elsewhere for three months, and the result is the finding that prompted this: Doug learned more in two hours from one derivation written for him than from months of lab work, because **the labs assume mathematics he had not been given, and no textbook supplies it in this order, at this moment, against this instrument.**

**Cellier remains the textbook author, and that is not the same role.** A textbook is written for every reader and sequenced by its subject. A lecture is written for one student and sequenced by what he is about to need. The division of teaching labour Decision 15 invoked is unchanged; only the seat Claude occupies in it has widened.
