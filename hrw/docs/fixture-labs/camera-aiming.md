...
that Claude cannot see: whether the camera actually lands where a link says.
...
run by `fixture_lab_links_all_resolve`, so a vocabulary change breaks the build rather than
breaking this file quietly.

---

## Station 1 — Load, and note where the camera starts
...
## Station 3 — Aim at the far corner
...
equation 999. A lab that names a missing equation is a bug in the lab, so it must be
visible rather than silently aiming somewhere plausible.

## Station 5 — Aiming survives a resize
