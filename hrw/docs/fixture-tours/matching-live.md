...
**So why does this tour print `189` at all?** Because something checks it.
`matching_ledger::every_line_the_live_tour_cites_is_a_real_anchor` scans this document for every
`matching.rs:<n>` and fails unless `<n>` is a live anchor or emit site. A citation is safe exactly
when a test resolves it — which is the same rule as the equation ids in the pass-one tours, and the
