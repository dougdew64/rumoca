...
**64 modules, 72,141 lines**, largest first. Every `.rs` file under `src/` at any depth, including the test-only ones (`ui_tests.rs`, `test_support.rs`); a module in a subdirectory is keyed by its path relative to `src/`.

| module | lines |
|---|---:|
| `app.rs` | 6,577 |
| `worker/tests.rs` | 6,566 |
| `app/tests.rs` | 6,177 |
