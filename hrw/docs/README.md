...
| [`reading-budgets.txt`](reading-budgets.txt) | **Data, not prose.** The line-count budgets `doc_citations::the_mandatory_reading_path_stays_small` enforces, plus the reasoning for every raise each has taken. Moved out of `src/` on 2026-08-31 so that raising one stays a docs-only commit and gates in ~6 s rather than ~170. |
