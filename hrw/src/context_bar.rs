...
pub(crate) enum PointKind {
    ...
    LabPassage {
        /// Which lab was open when the selection was made.
        ///
        /// **Not necessarily the document the text is in** — see `in_source`.
        lab: String,
        /// Whether the selected text was actually found in that lab's source.
        ///
        /// **The 🎯 press copies whatever is selected anywhere in the window**
        /// (`App::arm_lab_passage_capture` pushes `egui::Event::Copy`), and then labels
        /// it with whichever lab is open. So a selection made in a *stage pane* arrives
        /// here looking exactly like lab prose. Doug did it on 2026-09-22, capturing
        /// `flow sum equation: src.p.i + R.p.i = 0` — a string generated by
        /// `connection_anim`, not written in `connect-expansion.md` — and the emitted
        /// context told Claude to find it in that file.
        ///
        /// Checked at capture rather than asserted, so the claim `focus.json` makes is
        /// one that was tested. The text and the bar were right throughout; only the
        /// attribution was wrong, which is why this is a flag and not a new variant.
        in_source: bool,
    },
}
