...
/// **A pane selection is not a lab passage, and the emitted context must not say it is.**
///
/// # The capture this was written for
///
/// Doug selected `flow sum equation: src.p.i + R.p.i = 0` in the Connections pane on
/// 2026-09-22, with `connect-expansion` open, and pressed 🎯. The string is generated by
/// `connection_anim` and appears in no lab; `focus.json` nevertheless carried
/// `file: hrw/docs/fixture-labs/connect-expansion.md` and told Claude to locate it there.
/// **The copy and the Context Bar were both correct** — Doug confirmed the bar named
/// exactly what he had selected — so the defect was only ever the attribution, which is
/// why the fix is a checked flag rather than a new capture shape.
///
/// # Why the normalisation is the load-bearing half
///
/// egui returns what the pane *displays*, so a genuine passage arrives with its markdown
/// gone. A raw `contains` against the source would call almost every real passage foreign
/// and fire this warning constantly — the failure mode that would have made the fix worse
/// than the defect. Both arms are asserted here for that reason, the positive one using a
/// sentence whose source is thick with markup.
#[test]
fn a_pane_selection_is_not_mistaken_for_lab_prose() {
    let source = "# Flatten — what `connect` actually means

**A connector is a bundle of variables that two components meet at.** `RcCircuit` wires four
components together and never names a variable; it names `src.p`, `R.p`, `C.n` — see
[`expand_connector_connection`](hrw://src/crates/rumoca-phase-flatten/src/connections/mod.rs#expand_connector_connection).
";

    // Rendered prose: bold gone, backticks gone, link shown as its text, lines rewrapped.
    assert!(
        App::passage_is_in_lab(
            "A connector is a bundle of variables that two components meet at. RcCircuit wires              four components together",
            Some(source),
        ),
        "a real passage arrives with its markup stripped and its lines rewrapped; if that          reads as foreign, every capture would carry the warning and the warning would stop          meaning anything",
    );

    // The link's visible text survives; its target does not.
    assert!(
        App::passage_is_in_lab("see expand_connector_connection", Some(source)),
        "a link displays as its text, so that is what a selection across one returns",
    );

    // The capture that prompted all of this.
    assert!(
        !App::passage_is_in_lab("flow sum equation: src.p.i + R.p.i = 0", Some(source)),
        "this is `connection_anim` output selected from a pane, and calling it prose sends          Claude to edit a file that does not contain it",
    );

    // No lab loaded is not evidence of foreignness.
    assert!(
        App::passage_is_in_lab("anything at all", None),
