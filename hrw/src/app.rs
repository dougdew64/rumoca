            ...
            "context": {
                ...
                "pointing_at": self.context.pointed_at.as_ref().map(|p| json!({
                    ...
                    "kind": match &p.kind {
                        ...
                        // **`in_source` changes the noun, not just a flag.** "lab
                        // passage in X" about a string the lab does not contain is the
                        // same false claim the emitted context used to make.
                        PointKind::LabPassage { lab, in_source: true } => {
                            format!("lab passage in {lab}")
                        }
                        PointKind::LabPassage { lab, .. } => {
                            format!("pane selection, read while {lab} was open")
                        }
                    },
...
impl App {
    ...
    fn emit_context(&mut self) {
        ...
        if let PointKind::LabPassage { lab, in_source } = &point.kind {
            let ask = Ask {
                seq: point.seq,
    ...
    fn capture_lab_passage(&mut self, lab: String, text: String) {
        ...
        let in_source = Self::passage_is_in_lab(&text, self.lab.text());
        ...
        self.context.pointed_at = Some(PointedAt {
            ...
            target: text,
            kind: PointKind::LabPassage { lab, in_source },
