...
impl App {
    ...
    fn specimen_tree_options(&self) -> tree::TreeOptions<'_> {
        tree::TreeOptions {
            holding_selection: self.last_selection.is_some(),
    ...
    ///
    /// **`MenuOpened` must clear the slot before pushing the copy.** The sink is shared with
    /// ordinary Ctrl+C, so text left in it by an earlier copy would otherwise be handed to this
    /// gesture as though the reader had selected it — a point made of the wrong words, with
    /// nothing to reveal it. Clearing first means any text that arrives came from this press.
    ///
    /// The copy is pushed here and not in `pointing` because the sink is `App`'s: the helper
    /// reports, `App` performs, which is why every pane can opt in without touching the
    /// plumbing.
    fn perform_pointing(
        &mut self,
        ctx: &egui::Context,
        event: Option<crate::pointing::PointingEvent>,
    ) {
    ...
    /// What the lab panel is currently drawing, as an origin.
    ///
    /// **The two cases are not interchangeable, which is the whole reason this exists.** A
    /// fixture lab is a file in the repository and can be located and edited; Claude's answer
    /// is regenerated per question, gitignored, and in no file at all. `LabSource::label`
    /// returns `✨ Answer` for the second, so treating them alike emitted
    /// `file: hrw/docs/fixture-labs/✨ Answer.md` — a path that has never existed, and one the
    /// containment check cannot catch, because the text really is in the document on screen.
    fn lab_panel_origin(&self) -> Option<crate::pointing::PointOrigin> {
        match self.lab.selected.as_ref()? {
            LabSource::Answer => Some(crate::pointing::PointOrigin::Answer),
            source @ LabSource::Fixture(_) => Some(crate::pointing::PointOrigin::LabProse {
                lab: source.label(),
            }),
        }
    }

    /// Is this selected text actually in that lab's source?
    ///
    /// **The 🎯 press copies whatever is selected anywhere in the window**, then labels
    /// it with whichever lab is open, so a selection made in a stage pane arrives
    /// looking exactly like lab prose. Doug captured
    /// `flow sum equation: src.p.i + R.p.i = 0` on 2026-09-22 — generated by
    /// `connection_anim`, present in no lab — and `focus.json` told Claude to find it in
    /// `connect-expansion.md`. The copy and the bar were both right; only the
    /// attribution was wrong.
    ///
    /// **The comparison has to be normalised or it would reject real prose.** egui hands
    /// back what the pane *displays*, so `**bold**` arrives as `bold` and
    /// `` `code` `` as `code`; a raw `contains` against the markdown would call almost
    /// every genuine passage foreign. Both sides are stripped of emphasis and code
    /// marks, link targets are dropped so `[a](b)` compares as `a`, and whitespace is
    /// collapsed — the same flattening the bar already does for display.
    ///
    /// Errs toward *true*: with no lab text loaded there is nothing to contradict, and
    /// claiming prose is foreign would be its own false statement.
    fn passage_is_in_lab(text: &str, source: Option<&str>) -> bool {
        ...
        fn flatten(raw: &str) -> String {
            ...
            while let Some(c) = chars.next() {
                match c {
                    '*' | '`' | '_' | '#' | '>' => {}
                    // `[a](b)` displays as `a`, so drop the bracket and the target.
                    '[' | ']' => {}
                    '(' => {
                        // Only a link target follows `](`; an ordinary parenthesis is
                        // kept. Distinguished by what preceded it.
                        if out.ends_with(|c: char| c.is_alphanumeric() || c == ' ') {
                            out.push('(');
                        } else {
        ...
        needle.is_empty() || flatten(source).contains(&needle)
    }

    /// Make selected text the point, and emit it.
    ///
    /// `origin` is declared by the region that drew the text, so this never has to guess where
    /// a selection came from — the guess is what named a file for Claude's answer document and
    /// for compiler output alike.
    fn point_at_selection(&mut self, origin: crate::pointing::PointOrigin, text: String) {
        ...
        // **Only meaningful where the origin names a file.** Elsewhere there is nothing to
        // check the text against, and `false` would read as "we looked and it was not there".
        let in_source =
            origin.source_file().is_some() && Self::passage_is_in_lab(&text, self.lab.text());
        diagnostics::record_action("point-at-selection", origin.describe());
        self.context.jump_highlight = None;
        let seq = self.context.next_seq();
        self.context.pointed_at = Some(PointedAt {
    ...
    fn lab_panel_ui(&mut self, ui: &mut egui::Ui) -> Option<HrwLink> {
        ...
        let shown = self
            ...
            .show(ui, |ui| {
                ...
                // **Region 1 of `docs/pointing-plan.md` step 3, and the one Doug asked for
                // first:** *"I will definitely want to use the new point-at mechanism to
                // point at text in your answer documents. In fact, that is probably going to
                // be the most likely place that I will use that mechanism."* The answer and a
                // fixture lab draw in the same panel, so one wrapper serves both — and
                // `lab_panel_origin` is what keeps them apart, since only one of them is in a
                // file.
...
impl App {
    ...
    pub(crate) fn frame_ui(&mut self, ui: &mut egui::Ui) {
        ...
        // **The tree's own menu is where "Point at selection" has to live**, because a
        // `pointing::region` wrapped around this pane never opens: every right-click lands on
        // a row. The origin is the stage pane all the same — the text was drawn there — so the
        // capture is indistinguishable from one made in any other pane, which is the point.
        if tree_actions.point_at_selection {
            let origin = crate::pointing::PointOrigin::StagePane {
                stage: self.stage,
                sub_view: sub_view_name_for(self.stage, &self.viewport).map(str::to_owned),
            };
            match self.last_selection.clone() {
                Some(text) => self.point_at_selection(origin, text),
                // The item is only shown while text is held, so reaching this means the
                // selection went away between the menu opening and the click.
                None => self.notify(
                    "\u{26a0} the selection is gone \u{2014} drag across the text again, then \
                     right-click it",
                ),
            }
        }
